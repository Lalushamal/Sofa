...
def add_to_cart(request, product_id):
    ...
    sub_total = (
        cart_item.sub_total()
    )  # Assuming you have a method named sub_total in your Cartitem model
...
def user_cart(request):
    if request.user.is_authenticated:
        cart, created = Cart.objects.get_or_create(
            cart_id=cart_id(request), user=request.user
        )
        ...
        context = {"total": 0, "quantity": 0, "cart_items": []}
            ...
            return JsonResponse(
                {
                    ...
                    "sub_total": sub_total,
                    "grandTotal": grandTotal,
                    "removed": False,  # Indicate that the product was not removed
                }
            )
...
def remove_cart(request, product_id):
    ...
    try:
        cart_item = Cartitem.objects.get(
            product=product, cart__cart_id=cart_id(request)
        )
        cart_item.delete()
...
def checkout(request, total=0, quantity=0, cart_items=None):
    ...
    try:
        ...
        if request.user.is_authenticated:
            cart_items = Cartitem.objects.filter(
                user=request.user, is_active=True, product__soft_delete=False
            )
        else:
            cart = Cart.objects.get(cart_id=cart_id(request))
            cart_items = Cartitem.objects.filter(
                cart=cart, is_active=True, product__soft_delete=False
            )
        ...
        try:
            ...
            if coupon.valid_from <= timezone.now() <= coupon.valid_to:
                if order.order_total >= coupon.minimum_amount:
                    ...
                    messages.warning(
                        request, "Coupon is not Applicable for Order Total"
                    )
            else:
                messages.warning(
                    request, "Coupon is not Applicable for the current date"
                )
...
def cash_on_delivery(request, order_id):
    ...
    wishlist_items = Wishlist.objects.filter(
        user=current_user,
        product__in=[order_product.product for order_product in order_products],
    )
